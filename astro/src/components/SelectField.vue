...
<template>
    <div class="flex flex-col ml-2">
        ...
        <select :placeholder="props.modelValue" :class="isOther ? 'bg-gray-100' : ''" :value="props.modelValue"
            ...
            <option v-for="item in props.list" :key="item" :value="item">{{ t(`forms.report.questions.${props.title}.choices.${item}`) }}</option>
            <option value="others">{{ t("forms.report.questions.other_choice") }}</option>
        </select>
