  ...
  <div class="flex flex-col">
    ...
    >
      ...
      <option v-for="item in list" :key="item" :value="item">
        {{ t(`forms.report.questions.${props.title}.choices.${item}`) }}
      </option>
      <option v-if="props.other" value="others">
        {{ t("forms.report.questions.other_choice") }}
      </option>
    </select>
    <div v-if="props.other" v-show="isOther" class="flex flex-col gap-2 mt-3">
      <label class="mt-2 mb-1" :for="`enter_here_${props.title}`">{{
        t("pages.report_form.enter_here")
      }}</label>
      <input
        type="text"
    ...
    <span
      v-show="!isOther"
      class="mt-1 text-sm font-normal text-gray-400 italic"
