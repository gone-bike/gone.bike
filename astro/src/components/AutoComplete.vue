...
<template>
    <div  @focusin="isOpen = !isOpen" class="relative w-full h-full">
        <div @focusin="isOpen = !isOpen" class="flex flex-col ml-2 relative">
            <label  @focus="isOpen = !isOpen" :for="props.title" class='mb-2 text-lg w-full'>{{ t(`forms.report.questions.${props.title}.title`)
            }}</label>
            <input ref='searchEl' class="z-10" @keydown.enter="function () { searchEnter() }"
            ...
            <div :class="['border-gray-400 mt-1 border top-20 z-50 absolute w-full bg-white', { 'hidden': !isOpen }]">
                <div v-for="(result, idx) in queryResults"
                id="here"
