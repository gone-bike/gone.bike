...
const props = defineProps<{ api: { key: string, value: string }[], modelValue: string, title: string }>()
...
<template>
    <div class="flex flex-col ml-2">
        <div :class="isOther ? 'pointer-events-none' : ''" @click="toggleOpen">
            <label :for="props.title" class='mb-2 text-lg w-full'>{{ t(`forms.report.questions.${props.title}.title`) }}</label>
            <div :class="['border border-[#6b7280] mt-2 relative w-full h-fit py-2 px-3', isOther ? 'bg-gray-100' : '']">
