...
const props = defineProps<{
  api: { key: string | number; value: string }[];
  title: string;
  listed: string;
  newItem: string;
  topOffset: number;
...
const queryResults = computed(() => {
  ...
  let tempresult = props.api
    .map((e) => e.value)
    .filter((q) => q.toLowerCase().includes(searchQuery.value.toLowerCase()));
  let result =
    startIdx.value === 0
      ? tempresult
      : tempresult.slice(startIdx.value, tempresult.length - 1);
  if (result.length === 0) {
...
watch(firstClick, function (first) {
  if (
    first &&
    (isMobile(navigator.userAgent).phone ||
    ...
    if (props.title === "bike_brand") {
      if (props.topOffset === 0) {
        ...
        emit(
          "update:topOffset",
          document
            .getElementById(`dropdown-label-${props.title}`)!
            .getClientRects()
            .item(0)!.top
        );
        window.scrollTo({
...
onMounted(() => {
   ...
   */
  searchEl.value = document.getElementById(
...
function searchEnter(result?: string) {
  ...
  if (result) {
    ...
    searchQuery.value =
      queryResults.value[focusElIdx.value] ?? prevQuery.value[focusElIdx.value];
  }
  searchEl.value?.blur();
  isOpen.value = false;
  let apiList = props.api.map((e) => e.value);
  let idx = apiList.indexOf(
    result
      ? result
      : queryResults.value[focusElIdx.value] ??
          prevQuery.value[focusElIdx.value]
  );
  if (idx > -1) {
...
<template>
  <div class="relative w-full h-full">
    <div class="flex flex-col relative w-full">
      <label
        :id="`dropdown-label-${props.title}`"
        :for="props.title"
        class="mb-2 text-lg w-full font-medium"
        >{{ t(`forms.report.questions.${props.title}.title`) }}</label
      >
      <div :id="`dropdown-${props.title}`" class="w-full">
        <input
          ...
          class="w-full block py-3 px-5 text-gray-900 border border-gray-300 rounded-lg bg-gray-50 text-sm sm:text-base focus:ring-primary-600 focus-within:ring-primary-600"
          @keydown.enter="searchEnter"
          @keydown.arrow-down="searchKeyDown"
          @keydown.arrow-up="searchKeyUp"
          @focusin="isOpen = true"
          :value="searchQuery"
          type="search"
          autocomplete="off"
        ...
        <div
          :class="[
            'border-gray-400 mt-1 border top-26 z-50 absolute w-full h-fit max-h-64 overflow-y-scroll bg-white rounded-lg',
            { hidden: !isOpen },
          ]"
        >
          <div
            ...
            id="here"
            :class="[
              'p-2 border-b-2 text-gray-600  flex gap-2 items-center cursor-pointer border-gray-300 hover:bg-gray-200 hover:text-gray-900 focus:bg-gray-200 focus:text-gray-900',
              {
                '!bg-gray-200 !text-gray-900': idx === focusElIdx,
                'font-semibold': result === searchQuery,
              },
            ]"
            :key="result"
            @click="searchEnter(result)"
            tabindex="0"
          >
            {{ result }}
          </div>
